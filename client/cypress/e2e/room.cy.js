describe('room workflow', () => {
  ...
  it('blocks management actions when the room password is wrong', () => {
    cy.registerByApi().then(({ token }) => {
      cy.createRoomByApi({ token, name: 'Sala senha errada', password: '12345' }).then((room) => {
        cy.createQuestionByApi({ token, roomCode: room.code, text: 'Pergunta protegida' });
    ...
    cy.contains('button', 'Confirmar').click();
    cy.contains('Senha da sala invalida.').should('be.visible');
    cy.contains('Aberta').should('be.visible');
  });

  it('deletes a question and then deletes the room with the room password', () => {
    cy.registerByApi().then(({ token }) => {
      cy.createRoomByApi({ token, name: 'Sala para excluir', password: '12345' }).then((room) => {
        cy.createQuestionByApi({ token, roomCode: room.code, text: 'Pergunta para excluir' });
        cy.visitAs(`/room/${room.code}`, token);
      });
    });
