describe('admin pages', () => {
  it('shows admin links in the user menu and lists users', () => {
    cy.getAdminSession().then(({ token, user }) => {
      if (user.role !== 'admin') {
        cy.log('Skipping admin UI check: provide CYPRESS_adminEmail/CYPRESS_adminPassword or run against a clean database.');
        return;
      }
  ...
  it('lists rooms and filters them by user', () => {
    cy.getAdminSession().then(({ token, user }) => {
      if (user.role !== 'admin') {
        cy.log('Skipping admin room check: provide CYPRESS_adminEmail/CYPRESS_adminPassword or run against a clean database.');
        return;
      }

      cy.createRoomByApi({ token, name: 'Sala Admin Cypress', password: '12345' });
  ...
  it('redirects regular users away from admin routes', () => {
    cy.registerByApi().then(({ token, user }) => {
      if (user.role === 'admin') {
        cy.log('Skipping regular user redirect check: this database made the first E2E user an admin.');
        return;
      }
