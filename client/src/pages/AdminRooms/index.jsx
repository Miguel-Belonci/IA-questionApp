    ...
    <div className="admin-page">
      ...
      <main className="admin-main">
        <section className="admin-heading">
          ...
          <h1>Salas</h1>
          <p>Visualize todas as salas criadas e filtre por dono para acompanhar atividade.</p>
        </section>
        ...
        <section className="admin-card">
          <div className="admin-card-title">
            ...
            <Link className="secondary-button" to="/admin/users">Ver usuarios</Link>
          </div>

          <label htmlFor="user-filter">Filtrar por usuario</label>
          <select id="user-filter" value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)}>
                ...
                <tbody>
                  {rooms.map((room) => (
                    <tr key={room.id}>
                      <td>{room.name}</td>
                      <td>{room.code}</td>
                      <td>{room.owner?.name || 'Sem dono'}<span className="muted-line">{room.owner?.email}</span></td>
                      <td><span className={`status-pill ${room.owner?.active ? 'is-active' : 'is-inactive'}`}>{room.owner?.active ? 'Ativo' : 'Inativo'}</span></td>
                      <td>{room.questionsCount}</td>
                      <td>{room.openQuestionsCount}</td>
                      <td>{room.readQuestionsCount}</td>
                      <td>{new Date(room.createdAt).toLocaleDateString('pt-BR')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {rooms.length === 0 && <div className="table-empty">Nenhuma sala encontrada.</div>}
            </div>
          )}
        </section>
      </main>
