...
async function prepareSqliteSchema() {
  ...
  const hasRoomPasswordHash = roomsColumns.some((column) => column.name === 'passwordHash');

  if (!hasRoomPasswordHash) {
    await sequelize.query('ALTER TABLE `rooms` ADD COLUMN `passwordHash` VARCHAR(255);');
  }
}
