...
describe('API integration', () => {
  ...
  it('registers the first user as admin and prevents inactive users from logging in', async () => {
    ...
    await request(app)
      .post('/api/auth/register')
      .send({ name: 'Inactive', email: 'inactive@example.com', password: '123456' })
      .expect(201);

    const inactiveUser = await User.findOne({ where: { email: 'inactive@example.com' } });
  ...
  it('requires the room password to manage questions and rooms', async () => {
    const admin = await registerUser({ name: 'Owner', email: 'owner@example.com' });
    ...
    const deletedRoom = await Room.findOne({ where: { code: roomResponse.body.room.code } });
  ...
  it('lets admins list users and rooms but never deactivate admin accounts', async () => {
    ...
    const usersResponse = await request(app).get('/api/admin/users').set(auth).expect(200);
    expect(usersResponse.body.users).toHaveLength(1);

    const roomsResponse = await request(app).get('/api/admin/rooms').set(auth).expect(200);
